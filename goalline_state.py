...
PLACEMENT_BOX_Y_OFFSET = 3800

SHOT_POWER = 0.85  ## Tilføjet for at gøre skuddene nemmere (procent)

GOAL_LINE = 5100

YAW_MAX = np.pi

DEFENDER_IN_GOAL_FREQ = 3 # 1 out of X times

BOOST_MIN = 0.2

class GoaliePracticeState(StateSetter):

    def __init__(self, aerial_only=False, allow_enemy_interference=False, first_defender_in_goal=True,
    ...
    def _reset_cars(self, state_wrapper: StateWrapper, team_turn, first_defender_in_goal, allow_enemy_interference,
        ...
        :param reset_to_max_boost: Boolean indicating whether the cars will start each episode with 100 boost or keep from last episode
        """
        defender_in_goal_pick = random.randrange(1, DEFENDER_IN_GOAL_FREQ + 1)
        ...
        for car in state_wrapper.cars:
            ...
            if car.team_num == team_turn and not first_set:
                if first_defender_in_goal and defender_in_goal_pick == 1:
            ...
            if car.team_num == 0:  ## Adding yaw for the cars to face the ball
                car.set_rot(0, (rand.random() * 0.8 + 0.1) * np.pi, 0)
            else:
                car.set_rot(0, (rand.random() * 0.8 - 0.9) * np.pi, 0)

            if reset_to_max_boost:
                car.boost = 100
            else:
                car.boost = BOOST_MIN + (1 - BOOST_MIN) * rand.random()
    ...
    def _place_car_in_box_area(self, car, team_delin):
        ...
        y_pos = (PLACEMENT_BOX_Y / 2 - (rand.random() * PLACEMENT_BOX_Y))
    ...
    def _get_shot_parameters(self, team_turn, aerial_only):
        ...
        shot_randomizer = 1 if aerial_only else (random.uniform(.6, 1)) * SHOT_POWER
        ...
        elif shotpick == 4:  # my own close shot in the top
            z_pos = 0
            y_pos = 1200 * INVERT_IF_BLUE

            x_vel = 0
            y_vel = (1300 + 200 * rand.random()) * INVERT_IF_BLUE
            z_vel = 300 * rand.random() + 820
