...
PLACEMENT_BOX_X = 7000
PLACEMENT_BOX_Y = 2000
PLACEMENT_BOX_Y_OFFSET = 2800

SHOT_POWER = 0.5  ## Tilføjet for at gøre skuddene nemmere (procent)
...
class RecoveryState(StateSetter):

    def __init__(self, aerial_only=False, allow_enemy_interference=True, first_defender_in_goal=True,
                 reset_to_max_boost=False): ## Har sat enemy_interference til, fordi jeg vil have begge i samme ende
        """
        ...
        for car in state_wrapper.cars:
            ...
            if car.team_num == team_turn and not first_set:
                if first_defender_in_goal:
                    ...
                    self._place_defending_car_in_other_half(car, team_turn)
                ...
                if allow_enemy_interference:
                    ...
                    self._place_defending_car_in_other_half(car, car.team_num)

            if reset_to_max_boost:
                car.boost = 100

            car.set_rot(0, rand.random() * YAW_MAX - YAW_MAX / 2, 0)
            car.boost = rand.random() * (CAR_BOOST_MAX - CAR_BOOST_MIN) + CAR_BOOST_MIN ## Boost på random

    def _place_defending_car_in_other_half(self, car, team_delin):
        """
    ...
    def _get_shot_parameters(self, team_turn, aerial_only):
        ...
        shot_randomizer = 1 if aerial_only else (random.uniform(.8, 1)) * SHOT_POWER

        ### Har inverted alle y_vel, så boldene ryger den anden vej, da det skal træne recovery

        y_vel = (-3000 * INVERT_IF_BLUE) if aerial_only else (-3000 * shot_randomizer * INVERT_IF_BLUE)
        if shotpick == 0:  # long range shot

            y_pos = 3300 * INVERT_IF_BLUE
            z_pos = 1500 if aerial_only else random.uniform(100, 500)

            pos = np.array([x_pos, y_pos, z_pos])
            lin_vel = np.array([0, y_vel, 600])
        elif shotpick == 1:  # medium range shot

            y_pos = 500 * INVERT_IF_BLUE
            z_pos = 1550 if aerial_only else random.uniform(100, 550)
            ...
            x_vel = -1100 * SHOT_POWER
            y_vel = -2500 * INVERT_IF_BLUE * SHOT_POWER
            ...
            y_pos = 2000 * INVERT_IF_BLUE  ### Der var fejl på dette skud nr 3, nogle fortegn der skulle ændres, er fixet
